...
function plot_ocv_fit(ecm, gpm, df, id; correct_soc=true, plot_gp=true)
    fig = Figure()
    ax1 = Axis(fig[1, 1])
    ...
    if plot_gp
        @unpack gp, dt = gpm
        ŝ = StatsBase.transform(dt.s, (s * cap_real) .- soc0)
        ...
        band!(ax1, c1, μ - σ, μ + σ; label="GP", color=(Makie.wong_colors()[2], 0.3))
    end
    ...
    hist!(ax2, df_train.s .+ soc0, color=(:gray, 0.3), label="SOC")
...
function plot_rint_fit(df)
    ...
    errorbars!(ax, df.r_cu, μ, σ; whiskerwidth=5, color=Makie.wong_colors()[2])
...
function plot_soh_fit(df)
    ...
    errorbars!(ax, df.soh_cu, μ, σ; whiskerwidth=5, color=Makie.wong_colors()[2])
